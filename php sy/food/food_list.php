...
function do_edit(todo,code_no) {
	win_open("food_edit.php?todo="+todo+"&code_no="+code_no,"materialedit",600,480);
}
...
<table border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td><span class="header">메뉴 목록</span></td> <!-- 결과 -->
		<td>&nbsp; &nbsp;&nbsp; &nbsp;
...
if($search_word) {  //검색어가 있을 경우
	$sql_where .= " and (concat(no) like '%$search_word%' or UPPER(name) like '%$search_word%')	";
}

//where 1=1 and cancat(no) like '%배추%' and UPPER(name) like '%배추%'

$sql_orderby = $orderby ? 'order by '.$orderby : 'order by name ';
...
<tr>
	...
	<th>메뉴명 <a href="javascript:do_sort('name')">▲</a></th>
	<th>칼로리 <a href="javascript:do_sort('cal')">▲</a></th>
	<th>알러지 </th>
</tr>
